...
void startframe_recevied(LEUART_TypeDef *leuart);
...
void startframe_recevied(LEUART_TypeDef *leuart){
  ...
  case receive_data:
  case idle:
    leuart->IEN |= LEUART_IEN_SIGF | LEUART_IEN_RXDATAV;
    break;
  case close_uart:
  case transmit:
  case configure:
...
void sigframe_recevied(LEUART_TypeDef *leuart){
  ...
  case receive_data:
    ...
    leuart->CMD = LEUART_CMD_RXBLOCKEN | LEUART_CMD_CLEARRX;
